...
import com.kameocode.anydao.wraps.between
import com.kameocode.anydao.wraps.greaterThan
import com.kameocode.anydao.wraps.lessThan
import com.kameocode.anydao.wraps.lessThanOrEqualTo
...
class DatesTest : BaseTest() {
    ...
    fun `should search for Date (timestamp)`() {
        val u1 = UserODB(email = "email1", task = TaskODB(name = "task1"), timestamp = Date(), timestampNullable = Date(0), timestampNullable2 = Date(2))
        val u2 = UserODB(email = "email2", task = TaskODB(name = "task1"), timestamp = Date(1), timestampNullable = Date(0), timestampNullable2 = Date(2))
        anyDao.persist(u1, u2)
        ...
        val res5 = anyDao.all(UserODB::class) {
            it[UserODB::timestamp] between (it[UserODB::timestampNullable] to it[UserODB::timestampNullable2])
        }
