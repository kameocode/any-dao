...
import com.kameocode.anydao.wraps.abs
import com.kameocode.anydao.wraps.avg
import com.kameocode.anydao.wraps.diff
import com.kameocode.anydao.wraps.ge
import com.kameocode.anydao.wraps.gt
import com.kameocode.anydao.wraps.le
import com.kameocode.anydao.wraps.lt
import com.kameocode.anydao.wraps.max
import com.kameocode.anydao.wraps.min
import com.kameocode.anydao.wraps.mod
import com.kameocode.anydao.wraps.neg
import com.kameocode.anydao.wraps.prod
import com.kameocode.anydao.wraps.quot
import com.kameocode.anydao.wraps.sqrt
import com.kameocode.anydao.wraps.sum
...
class NumberTest : BaseTest() {
    ...
    @Test
    fun `should support less than and greater than predicates`() {
        val u1 = UserODB(email = "email1", task = TaskODB(name = "task1"), counter = 1, counterNullable = 1, counterDouble = 1.0, counterDoubleNullable = 1.0)
        val u2 = UserODB(email = "email2", task = TaskODB(name = "task3"), counter = 2, counterNullable = 2, counterDouble = 2.0, counterDoubleNullable = 2.0)
        val u3 = UserODB(email = "email2", task = TaskODB(name = "task3"), counter = 3, counterDouble = 3.0)
        anyDao.persist(u1, u2, u3)
        ...
        val res1: List<UserODB> = anyDao.all(UserODB::class) {
            it[UserODB::counter] le 2
        }
        Assert.assertEquals(setOf(u1.id, u2.id), res1.map { it.id }.toSet())

        val res2: List<UserODB> = anyDao.all(UserODB::class) {
            it[UserODB::counter] lt 2
        }
        Assert.assertEquals(setOf(u1.id), res2.map { it.id }.toSet())

        val res3: List<UserODB> = anyDao.all(UserODB::class) {
            it[UserODB::counter] ge 2
        }
        Assert.assertEquals(setOf(u2.id, u3.id), res3.map { it.id }.toSet())

        val res4: List<UserODB> = anyDao.all(UserODB::class) {
            it[UserODB::counter] gt 2
        }
        Assert.assertEquals(setOf(u3.id), res4.map { it.id }.toSet())
    }
}